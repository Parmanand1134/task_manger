...
const Sidebar = () => {
  ...
  const projectData = () => {
    ...
    axios.get('http://localhost:9000/projects/', { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
      .then((res) => {
        setProjects(res.data);
      })
      .catch((error) => {
        console.error('Error fetching project data:', error);
        // Handle error: show error message to the user or retry fetching data
      });
  };
      ...
      <ul className='border-r border-gray-300 pr-2'>
        {projects.map((project, index) => (
          <Link key={index} to={`/${project._id}`} onClick={(e) => handleLocation(e)}>
            <li className={`px - 10 py - 5.5 mb - 3 text - gray - 600 font text - sm capitalize select - none hover: text - indigo - 600 rounded transition - colors hover: bg - indigo - 200 / 80 ${paramsWindow === project._id && 'text-indigo-600 bg-indigo-200/80'}`}>
              {project.title}
            </li>
          </Link>
        ))}
