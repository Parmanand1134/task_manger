...
function Task() {
    ...
    useEffect(() => {
        ...
        const filteredData = Object.fromEntries(Object.entries(columns).map(([columnId, column]) => {
            return [
                columnId,
                {
                    ...column,
    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            ...
            axios.get(`http://localhost:9000/project/${projectId}`, { headers })
                .then((res) => {
    ...
    const updateTodo = (data) => {
        axios.put(`http://localhost:9000/project/${projectId}/todo`, data, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
            .then((res) => {
            }).catch((error) => {
                toast.error('Something went wrong')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`http://localhost:9000/project/${projectId}/task/${taskId}`, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
            .then((res) => {
