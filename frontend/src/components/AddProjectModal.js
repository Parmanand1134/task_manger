...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    useEffect(() => {
        if (edit && isModalOpen) {
            axios.get(`http://localhost:9000/project/${id}`, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
                .then((res) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post('http://localhost:9000/project/', { title, description: desc }, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
                .then((res) => {
                    closeModal()
            ...
            axios.put(`http://localhost:9000/project/${id}`, { title, description: desc }, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
                .then((res) => {
                    closeModal()
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
