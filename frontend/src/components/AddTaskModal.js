...
const AddTaskModal = ({ isAddTaskModalOpen, setAddTaskModal, projectId = null, taskId = null, edit = false, refreshData }) => {
    ...
    useEffect(() => {
        if (edit && isAddTaskModalOpen) {
            axios.get(`http://localhost:9000/project/${projectId}/task/${taskId}`, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
                .then((res) => {
    ...
    // useEffect(() => {
    //     axios.get('http://localhost:9000/users', { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
    //         .then((res) => {
    //             setUserEmails(res.data.map(user => user.email));
    //         })
    //         .catch((error) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`http://localhost:9000/project/${projectId}/task`, { title, description: desc }, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
                .then((res) => {
            ...
            axios.put(`http://localhost:9000/project/${projectId}/task/${taskId}`, { title, description: desc }, { headers: { 'Authorization': `Bearer ${Cookies.get('token')}` } })
                .then((res) => {
                    setAddTaskModal(false)
                        ...
                        >
                            <Dialog.Panel className="rounded-md bg-white w-6/12">
                                ...
                                <form onSubmit={handleSubmit} className='gap-4 px-8 py-4'>
                                    ...
                                    {/* 
                                    <div className='mb-3'>
                                        <label htmlFor="assign" className='block text-gray-600'>Assign to</label>
                                        <select value={userId} onChange={(e) => setUserId(e.target.value)} className='border border-gray-300 rounded-md w-full text-sm py-2 px-2.5 focus:border-indigo-500 focus:outline-offset-1 focus:outline-indigo-400'>
